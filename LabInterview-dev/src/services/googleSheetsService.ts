...
export const findExistingSpreadsheet = async (token: string, title: string): Promise<string | null> => {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 || response.status === 401) {
      if (errorData.error?.message?.includes('Google Drive API')) {
        throw new Error('DRIVE_API_DISABLED');
      }
      if (reason === 'ACCESS_TOKEN_SCOPE_INSUFFICIENT' || errorData.error?.message?.includes('insufficient authentication scopes')) {
        // 如果權限不足，清除舊 Token 並要求重登
